...
Eso condiciona el recorrido, que va de dentro hacia fuera en cinco partes:
...
Si venís del mundo del dato o del desarrollo y lo que queréis es construir, la ruta corta es [qué es un agente](agentes/queesunagente.qmd), [herramientas](agentes/herramientas.qmd) y [evaluación](produccion/evaluacion.qmd), volviendo a la primera parte cuando algo no cuadre. Si lo que os toca es decidir si esto entra en vuestra organización, la ruta es [el ecosistema productivo](produccion/ecosistema.qmd), [los retos de seguridad](seguridad/retos.qmd) y [la normativa](normativa/leyes.qmd).

Y si el interés es entender de verdad qué está pasando, entonces sí: empezad por [el principio](modelos/mlgenerativa.qmd), que es donde se explica por qué un sistema que no sabe nada de nada resulta ser tan útil.
